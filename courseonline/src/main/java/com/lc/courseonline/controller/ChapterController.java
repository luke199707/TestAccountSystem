package com.lc.courseonline.controller;


import com.baomidou.mybatisplus.core.conditions.query.QueryWrapper;
import com.lc.courseonline.commons.utils.LayuiTableData;
import com.lc.courseonline.commons.utils.UUIDUtil;
import com.lc.courseonline.entity.Chapter;
import com.lc.courseonline.service.ChapterService;
import com.lc.courseonline.service.SubsectionService;
import io.swagger.annotations.Api;
import io.swagger.annotations.ApiOperation;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;
import org.springframework.web.servlet.ModelAndView;

import java.util.HashMap;
import java.util.List;
import java.util.Map;

/**
 * <p>
 *  前端控制器
 * </p>
 *
 * @author zhangjunhui
 * @since 2022-07-15
 */
@Api(description = "章节管理")
@RestController
@RequestMapping("/courseonline/chapter")
public class ChapterController {

    @Autowired
    private ChapterService chapterService;
    @Autowired
    private SubsectionService subsectionService;

    @ApiOperation(value = "章节添加")
    @PostMapping("add")
    public ModelAndView add(String courseid, String title, String introduction){
        ModelAndView mav = new ModelAndView();
        //封装数据
        Chapter chapter = new Chapter();
        chapter.setId(UUIDUtil.getUUID());
        chapter.setCourseid(courseid);
        chapter.setTitle(title);
        chapter.setIntroduction(introduction);
        //执行Dao
        chapterService.save(chapter);
        //返回
        mav.setViewName("redirect:/backlink/chapter");//重定向到跳转BackLinkController，避免页面刷新表单重复提交
        return mav;
    }

    @ApiOperation(value = "章节删除")
    @GetMapping("deleteById")
    @ResponseBody
    public String deleteById(String id){
        //先删除小节信息
        subsectionService.deleteSubsectionByChapterid(id);
        //再删除章节信息
        if (chapterService.removeById(id)) {
            return "success";
        }
        return "fail";
    }

    @ApiOperation(value = "章节修改")
    @PostMapping("edit")
    public ModelAndView edit(String id,String courseid, String title, String introduction){
        ModelAndView mav = new ModelAndView();
        //封装数据
        Chapter chapter = chapterService.getById(id);
        chapter.setCourseid(courseid);
        chapter.setTitle(title);
        chapter.setIntroduction(introduction);
        //执行Dao
        chapterService.updateById(chapter);
        //返回
        mav.setViewName("redirect:/backlink/chapter");//重定向到跳转BackLinkController，避免页面刷新表单重复提交
        return mav;
    }

    @ApiOperation(value = "章节分页条件查询")
    @GetMapping("pageListConditionChapter")
    public LayuiTableData pageListConditionChapter(long page, long limit, String title,
                                          @RequestParam(required = false,defaultValue = "create_time") String sortField,
                                          @RequestParam(required = false,defaultValue = "asc") String order){
        //创建layui数据表格专用返回对象
        LayuiTableData layuiTableData = new LayuiTableData();
        //封装list查询条件
        Map<String, Object> map = new HashMap<>();
        map.put("title", title);
        map.put("sortField", sortField);
        map.put("order", order);
//        map.put("page", (page - 1) * limit);//使用offset关键字分页时使用
        map.put("page", page);
        map.put("limit", limit);
        List<Chapter> data = chapterService.queryChapterByConditionForPage(map);
        int total = chapterService.queryCountOfChapterByCondition(map);
        //返回数据封装
        layuiTableData.setCode(0);
        layuiTableData.setMsg("");
        layuiTableData.setCount(total);
        layuiTableData.setData(data);
        //返回
        return layuiTableData;
    }

    @ApiOperation(value = "通过课程id获取章节")
    @GetMapping("getByCourseid")
    @ResponseBody
    public List<Chapter> getByCourseid(String courseid) {
        QueryWrapper<Chapter> wrapper = new QueryWrapper<>();
        wrapper.orderByAsc("create_time");
        wrapper.eq("courseid",courseid);
        List<Chapter> chapterList = chapterService.list(wrapper);
        return chapterList;
    }

    @ApiOperation(value = "通过章节名称和课程id获取课程id")
    @GetMapping("getIdByTitleCourseid")
    @ResponseBody
    public String getIdByTitleCourseid(String title,String courseid) {
        QueryWrapper<Chapter> wrapper = new QueryWrapper<>();
        wrapper.eq("title",title);
        wrapper.eq("courseid",courseid);
        Chapter one = chapterService.getOne(wrapper);
        return one.getId();
    }

    @ApiOperation(value = "添加时确认参数")
    @GetMapping("addCheckParams")
    @ResponseBody
    public boolean  addCheckParams(String param, String value) {
        boolean result = chapterService.addCheckParams(param,value);
        return result;
    }

    @ApiOperation(value = "编辑时确认参数")
    @GetMapping("editCheckParams")
    @ResponseBody
    public boolean  editCheckParams(String param, String value,String id) {
        boolean result = chapterService.editCheckParams(param,value,id);
        return result;
    }

    //---------------------------------------------------前台使用---------------------------------------------------

    @ApiOperation(value = "获取所有章节")
    @GetMapping("getAllChapter")
    @ResponseBody
    public List<Chapter> getAllChapter(){
        QueryWrapper<Chapter> wrapper = new QueryWrapper<>();
        wrapper.orderByAsc("create_time");
        List<Chapter> chapterList = chapterService.list(wrapper);
        return chapterList;
    }
}

